...
const Prediction = () => {
    ...
    const availableModels = [
        { id: "LINEAR", name: "Linear Regression (Continuous Setup)" },
        { id: "LOGISTIC", name: "Logistic Regression (Classification Simulation)" }
    ];
    ...
    const fetchForecast = async () => {
        ...
        setError(null);
        try {
            const res = await stockService.getPredictionAnalysis(selectedStock.toUpperCase(), selectedModel.toLowerCase());
        ...
        <div className="p-4 md:p-8 max-w-[1700px] mx-auto animate-fade-in relative font-sans text-[#a4a9c3]">
            ...
            <div className="mb-10">
                <h1 className="text-4xl lg:text-5xl font-black text-white tracking-tighter mb-4 flex items-center gap-4">
                    Stock Prediction <span className="text-2xl font-bold bg-[#1e2029] text-cyan-400 px-4 py-1.5 rounded-full border border-cyan-500/20 tracking-wide">AI</span>
                </h1>
                <p className="text-lg text-[#8f95b2] max-w-4xl leading-relaxed tracking-wide font-medium">
                    Select any asset to extrapolate its pricing trajectory 30 days into the future. Compare continuous Linear Regression models against simulated Logistic Regression classification paths.
                </p>
            </div>
            ...
            {!error && (
                <div className="relative bg-[#0a0b0e] rounded-[2rem] border border-white/5 overflow-hidden shadow-2xl min-h-[600px] flex flex-col">
                    ...
                    <div className={cn(
                        ...
                        selectedModel === 'LINEAR' ? "bg-cyan-500" : "bg-rose-500"
                    )}></div>
                    ...
                    {loading ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-12">
                            <div className={cn(
                                ...
                                selectedModel === 'LINEAR' ? "border-cyan-500" : "border-rose-500"
                            )}></div>
                            <h3 className="text-2xl font-black text-white tracking-tight mb-2">Compiling Tensor Graphs...</h3>
                            <p className="text-[#8f95b2] font-medium max-w-md mx-auto">Allocating compute arrays to fit the {selectedModel} model on localized 5-year trajectories.</p>
                        </div>
                    ) : forecastData && (
                            ...
                            <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6 mb-12 border-b border-white/5 pb-8">
                                <div>
                                    <h2 className="text-3xl font-black text-white tracking-tight mb-2 flex items-center gap-3">
                                        <Target size={28} className={selectedModel === 'LINEAR' ? "text-cyan-400" : "text-rose-400"} />
                                        Prediction Output
                                    </h2>
                                    ...
                                    <AreaChart data={forecastData.chart} margin={{ top: 10, right: 0, left: 10, bottom: 0 }}>
                                        <defs>
                                            ...
                                            <linearGradient id="colorForecast" x1="0" y1="0" x2="0" y2="1">
                                                <stop offset="5%" stopColor={selectedModel === 'LINEAR' ? "#22d3ee" : "#f43f5e"} stopOpacity={0.4} />
                                                <stop offset="95%" stopColor={selectedModel === 'LINEAR' ? "#22d3ee" : "#f43f5e"} stopOpacity={0} />
                                            </linearGradient>
                                        </defs>
                                        ...
                                        <Area
                                            ...
                                            type="monotone"
                                            dataKey="Forecast"
                                            stroke={selectedModel === 'LINEAR' ? "#06b6d4" : "#e11d48"}
                                            fill="url(#colorForecast)"
                                            strokeWidth={3}
                                            strokeDasharray="5 5"
                                            activeDot={{ r: 8, fill: selectedModel === 'LINEAR' ? "#06b6d4" : "#e11d48", strokeWidth: 0, shadow: '0 0 10px yellow' }}
                                        />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
